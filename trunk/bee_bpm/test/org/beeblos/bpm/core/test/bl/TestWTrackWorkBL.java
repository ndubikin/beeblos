...
public class TestWTrackWorkBL extends TestCase {
	...
	public void testAgregarWTrackWork() throws Exception {
		...
		assertEquals(2, routeBL.getWStepSequenceDefs(iproc, version1, 40, 1001)
				.size());
		assertEquals(2, routeBL.getWStepSequenceDefs(iproc, version1, 20, 1001)
				.size());
		assertEquals(1, routeBL.getWStepSequenceDefs(iproc, version1, 10, 1001)
				.size());
