...
public class TestWStepSequenceDefBL extends TestCase{
		...
		public void testRecuperarWStepSequenceConWExternalMethods() {
			...
			iroute = 7;
			WStepSequenceDef stepsequence = null;
			try {
				stepsequence = new WStepSequenceDefBL().getWStepSequenceDefByPK(iroute, 1000);
			} catch (WStepSequenceDefException e) {
				e.printStackTrace();
			}
			
			System.out.println("EXTERNAL METHODS: " 
					+ ((stepsequence.getExternalMethod() != null)?stepsequence.getExternalMethod().size():"null"));
